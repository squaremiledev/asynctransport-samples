 ...
 return SessionRejectReason.NULL_VAL;
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        if (hasText())
        {
            indent(builder, level);
            builder.append("\"Text\": \"");
            builder.append(text(), 0, textLength());
            builder.append("\",\n");
        }
    ...
    {
        encoder.reset();
        encoder.refSeqNum(refSeqNum());
        if (hasRefTagID())
        {
            encoder.refTagID(refTagID());
        }
        ...
        if (hasSessionRejectReason())
        {
            encoder.sessionRejectReason(sessionRejectReason());
        }

        if (hasText())
        {
            encoder.text(text(), 0, textLength());
        }
        ...
        if (hasEncodedText())
        {
            encoder.encodedTextAsCopy(encodedText(), 0, encodedTextLen());
            encoder.encodedTextLen(encodedTextLen());
        }
        return encoder;
    }
